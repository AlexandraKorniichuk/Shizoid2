...
#include "Shizoid2/AbilitySystem/Shizoid2AbilitySystemComponent.h"
#include "Shizoid2/AbilitySystem/Shizoid2AbilityAttributeSet.h"
#include "Shizoid2/AbilitySystem/AbilityInputBindingComponent.h"

#define LOCTEXT_NAMESPACE "Shizoid2Features"
...
void UGameFeatureAction_AddAbilities::AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData)
{
	if (UAssetManager::IsValid())
	{
		...
		{
			for (const FShizoid2AbilityMapping& Ability : Entry.GrantedAbilities)
			{
			...
			for (const FShizoid2AttributesMapping& Attributes : Entry.GrantedAttributes)
			{
...
void UGameFeatureAction_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry)
{
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry))
	{
		...
		for (const FShizoid2AbilityMapping& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FShizoid2AttributesMapping& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				TSubclassOf<UShizoid2AbilityAttributeSet> SetType = Attributes.AttributeSetType.LoadSynchronous();
				if (SetType)
				{
